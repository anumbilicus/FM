...
vector<pair<int,int>> Graph::add_to_bucket(CellPtr tmp,map<string,Tech> techs, vector<pair<int,int>> curr, DieRows topdierow, DieRows bottomdierow, int topdieutil, int bottomdieutil, int diearea){
	...
	tmp->len.first.first = techs[topdietech].cell_types[tmp->libCell].size.first;
	tmp->len.first.second = techs[topdietech].cell_types[tmp->libCell].size.second;
	tmp->len.second.first = techs[bottomdietech].cell_types[tmp->libCell].size.first;
	tmp->len.second.second = techs[bottomdietech].cell_types[tmp->libCell].size.second;
	...
	for(int i=0;i< this->total_cells;i++){
		...
		if(vertices[i]->used == 0 && vertices[i]->gain >= biggest_gain){
			...
			if(vertices[i]->bucket == 0){ //cell on top die
				tmp_area = (float)(this->bottom_area+vertices[i]->len.second.first*vertices[i]->len.second.second) / (float)(this->total_area); 
				//cout<<"tmp_ares:"<<this->bottom_area+vertices[i]->len.second.first*vertices[i]->len.second.second<<endl;
				...
				tmp_area = (float)(this->top_area+vertices[i]->len.first.first*vertices[i]->len.first.second) / (float)(this->total_area); 
				//cout<<"tmp_ares:"<<(this->top_area+vertices[i]->len.first.first*vertices[i]->len.first.second)<<endl;
...
void Graph::net_change(){
	...
	if(curr_bucket == 0){
		...
		this->top_area -= vertices[this->biggest_cell]->len.first.first * vertices[this->biggest_cell]->len.first.second;
		this->bottom_area += vertices[this->biggest_cell]->len.second.first * vertices[this->biggest_cell]->len.second.second;
	}
	else{
